...
from .models import User, LoginAttempts, RegistrationPIN
...
def webauthn_registration_start():
    ...
    if current_user.is_authenticated:
        ...
        print(pin)
        pin_results = RegistrationPIN.query.filter_by(pin=pin).first()

        if not pin_results:
            flash("Registration PIN is invalid", 'error')
    ...
    db.session.query(RegistrationPIN).filter(RegistrationPIN.pin==pin).delete(synchronize_session=False)
    db.session.commit()
...
def webauthn_verify_login():
    ...
    if user:
        ...
        #modified, not using this anymore
